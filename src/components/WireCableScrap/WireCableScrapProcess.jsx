...
import { Truck, Scissors, ArrowDownUp, PackageCheck, Banknote } from 'lucide-react';
import { motion, useScroll, useInView } from 'framer-motion';

const phases = [
  {
    number: 1,
    title: 'Collection, Sorting & Pre-Processing',
    description: 'We perform pan-India collection of bulk scrap directly from your location. Scrap is initially sorted manually into categories (like High-Grade Communications, Power Cable, Automotive) to optimize processing, and large cables are sheared to suitable lengths.',
    icon: Truck,
  ...
  {
    number: 2,
    title: 'Primary Granulation (Shredding)',
    description: 'The sorted cable is fed into our primary granulator. This machine uses high-speed rotating blades to chop the cables into small, uniform pieces (typically 10-20mm), successfully breaking the strong bond between the insulation and the metal conductor.',
    icon: Scissors,
  ...
  },
  {
    ...
    description: 'The chopped material undergoes multi-stage separation. Air Separation (Aspiration) sucks away low-density plastic fluff, while dense metal granules fall through. Electrostatic Separation is then used for up to 99.9% purity recovery.',
    icon: ArrowDownUp,
  ...
  {
    number: 4,
    title: 'Output Streams & Quality Control',
    description: 'The process generates three clean streams: 1) High-purity Copper/Aluminium granules free of plastic, 2) Clean Plastic Regrind for circular economy uses, and 3) A very small fraction of residual fines managed responsibly.',
    icon: PackageCheck,
  ...
  {
    number: 5,
    title: 'Weighing, Documentation & Payment',
    description: 'The output copper and plastic are weighed separately on certified scales. Based on the net weight of clean metal and prevailing market rates, we provide transparent and rapid payouts along with full disposal records.',
    icon: Banknote,
        ...
        <section id="roadmap-process" className="ewaste-process bg-white dark:bg-black !p-0 flex justify-center overflow-hidden">
            <div className="ewaste-process__container max-w-[1400px] w-full flex flex-col gap-4 md:gap-6">
                <div className="text-center flex flex-col items-center gap-2 md:gap-4">
                    ...
                    >
                        Our State-of-the-Art Granulation Process
                    </motion.h2>
                    <motion.p 
                        initial={{ opacity: 0, y: 20 }}
                        whileInView={{ opacity: 1, y: 0 }}
                    ...
                    >
                        Our process is mechanical, clean, and designed for maximum separation efficiency.
                    </motion.p>
                </div>
...
export default WireCableScrapProcess;
