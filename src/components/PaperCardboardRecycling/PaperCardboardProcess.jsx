...
import { ClipboardCheck, Truck, Cog, Recycle } from 'lucide-react';
import { motion, useScroll, useInView } from 'framer-motion';

const phases = [
  {
    number: 1,
    title: 'Customized Collection Program Setup',
    description: 'We design a waste management plan tailored to your operations, supplying color-coded bins for Mixed Paper, OCC, and Confidential Paper. We also provide initial staff training to ensure only clean, dry paper enters the recycling stream.',
    icon: ClipboardCheck,
  ...
  },
  {
    ...
    description: 'Our crew performs visual checks during scheduled pickups. Sensitive documents are sealed in security bags or locked consoles and transported in dedicated locked compartments to ensure absolute data security.',
    icon: Truck,
  ...
  {
    number: 3,
    title: 'Advanced Processing at our MRF',
    description: 'Cardboard (OCC) and mixed paper are sorted along conveyor belts to remove plastic and metals before being compressed into dense 500-700kg bales. Confidential paper is cross-shredded to unreadable confetti before baling.',
    icon: Cog,
  ...
  {
    number: 4,
    title: 'Quality Control & Supply to Mills',
    description: 'Every bale is inspected for purity and graded (e.g. #11 OCC, DLK, ONP). The finished bales are then supplied directly to paper and board mills across India to manufacture new boxes, tissue, and kraft paper.',
    icon: Recycle,
...
const PaperCardboardProcess = () => {
    ...
    return (
        <section id="roadmap-process" className="paper-cardboard-process bg-white dark:bg-black !p-0 flex justify-center overflow-hidden">
            <div className="paper-cardboard-process__container max-w-[1400px] w-full flex flex-col gap-4 md:gap-6">
                <div className="text-center flex flex-col items-center gap-2 md:gap-4">
                    ...
                    >
                        Comprehensive Recycling Lifecycle
                    </motion.h2>
                    <motion.p 
                        initial={{ opacity: 0, y: 20 }}
                        whileInView={{ opacity: 1, y: 0 }}
                    ...
                    >
                        We provide end-to-end solutions ensuring clean, segregated fiber is processed and channeled back to paper mills.
                    </motion.p>
                </div>
...
export default PaperCardboardProcess;
